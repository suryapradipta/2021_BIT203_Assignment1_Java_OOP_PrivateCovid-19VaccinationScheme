...
public class PCVS {
    ...
    // aggregasi healthcare centre dan batch
    ...
    // aggregasi Vaccine dan batch ditambah method diatas findHealthcareCentreBatch
    // belum selesai
    public int findVaccineBatch(int index) {
        Vaccine VC = getPCVSVaccines().get(index);
        for (int j = 0; j < VC.getBatches().size(); j++) {
            findHealthcareCentreBatch(VC.getBatches().get(j));
            return findHealthcareCentreBatch(VC.getBatches().get(j));
        }
        return -1;
    }
