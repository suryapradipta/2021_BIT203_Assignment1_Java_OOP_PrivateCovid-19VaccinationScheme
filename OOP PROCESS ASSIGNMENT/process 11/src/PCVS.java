...
public class PCVS {
   ...
   /* public String findBatch() {
    ...
    public Patient getPatient() {
        for (int i = 0; i < getPCVSUsers().size(); i++) {
            if (getPCVSUsers().get(i) instanceof Patient patient) {
                return (Patient) getPCVSUsers().get(i);
            }
        }
        return null;
    }
    ...
    // method finish temp
    public String findBatch(int iHC) {
        ArrayList<Batch> tempBatchCollection = new ArrayList<>();
        HealthcareCentre tempHC  = getPCVSHealthcareCentres().get(iHC);
        for(int i = 0; i < getPCVSVaccines().size(); i ++) {
        ...
        return Arrays.toString(tempBatchCollection.toArray())
            .replace("[", "")
            .replace("]", "")
            .replace(", ", "\n");
